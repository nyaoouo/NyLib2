"""VEH backend: in-process VEH dispatcher for hardware/software breakpoints.

The native side lives in ``veh_backend.cpp`` and is built into
``veh_backend.dll`` by :func:`build_backend`. The DLL exports the seven-function
backend ABI documented in docs/superpowers/specs/2026-05-19-breakpoint-design.md
(six per-spec entry points plus ``BpLastError`` for slot-exhaustion mapping).
"""
...
# Native trampoline signature: (bp_handle, flags, frm, context_addr).
...
_DEFAULT_DLL = _DIR / "veh_backend.dll"
_DEFAULT_CPP = _DIR / "veh_backend.cpp"


def build_backend(dst: str | pathlib.Path = _DEFAULT_DLL, build_dir=None) -> pathlib.Path:
    """Compile ``veh_backend.cpp`` into ``dst``.

    Mirrors :func:`nylib.winutils.python_loader.build_loader`.
    """
...
class VehBackend(BreakpointBackend):
    """ctypes wrapper around veh_backend.dll."""
    ...
    def __init__(self):
        ...
        self._bind()
        self._handle = self._dll.BpBackendInit(_BACKEND_VERSION)
        if self._handle == 0:
            raise BackendError("BpBackendInit returned 0 (see DebugView)")
        # native bp_handle -> {bp, trampoline, callback}
        self._handle_map: dict[int, dict] = {}
        # Keep CFUNCTYPE thunks alive for the lifetime of the BP.
        self._trampolines: list = []

    def _bind(self) -> None:
        self._dll.BpBackendInit.restype = ctypes.c_void_p
        self._dll.BpBackendInit.argtypes = [ctypes.c_uint32]
        self._dll.BpBackendShutdown.restype = ctypes.c_uint32
        self._dll.BpBackendShutdown.argtypes = [ctypes.c_void_p]
        self._dll.BpInstall.restype = ctypes.c_void_p
        self._dll.BpInstall.argtypes = [
            ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint32, ctypes.c_uint32,
            ctypes.c_void_p, ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_uint32), ctypes.c_uint32,
        ]
        self._dll.BpUninstall.restype = ctypes.c_uint32
        self._dll.BpUninstall.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        self._dll.BpEnable.restype = ctypes.c_uint32
        self._dll.BpEnable.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_uint32]
        self._dll.BpSlotsFree.restype = ctypes.c_uint32
        self._dll.BpSlotsFree.argtypes = [ctypes.c_void_p, ctypes.c_uint32]
        self._dll.BpLastError.restype = ctypes.c_uint32
        self._dll.BpLastError.argtypes = []
        self._dll.BpAttachTids.restype = ctypes.c_uint32
        self._dll.BpAttachTids.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_uint32), ctypes.c_uint32,
        ]
        self._dll.BpDetachTids.restype = ctypes.c_uint32
        self._dll.BpDetachTids.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_uint32), ctypes.c_uint32,
        ]
        self._dll.BpListTids.restype = ctypes.c_uint32
        self._dll.BpListTids.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_uint32), ctypes.c_uint32,
        ]
        self._dll.BpSnapshotTids.restype = ctypes.c_uint32
        self._dll.BpSnapshotTids.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_uint32), ctypes.c_uint32,
        ]
    ...
    def install(self, *, address, size, flags, callback, user_data, tids):
        ...
        # Per-install state captured by the trampoline closure.
        entry: dict = {"bp": None, "callback": callback}
        ...
        if tids:
            tid_arr_t = ctypes.c_uint32 * len(tids)
            tid_arr = tid_arr_t(*tids)
            tid_ptr = ctypes.cast(tid_arr, ctypes.POINTER(ctypes.c_uint32))
            tid_count = len(tids)
        else:
            tid_arr = None
            tid_ptr = ctypes.POINTER(ctypes.c_uint32)()
            tid_count = 0
        # keep tid_arr alive for the duration of the call
        _ = tid_arr
    ...
    def bind_bp(self, handle: int, bp) -> None:
        """Link a native BP handle back to the Python BreakPoint."""
    ...
    def uninstall(self, handle):
        ...
        # Note: we don't drop trampolines from self._trampolines (slot reuse
        # would be a pain) - they stay alive for backend lifetime.
        ...
        if rc != 0:
            ...
            if err == 3:
                raise SlotExhaustedError(
                    "HW DR slots exhausted on at least one requested TID"
                )
            if err == 6:
                raise BackendError("attach_tids: BP is SOFT (no per-TID slots)", status=err)
