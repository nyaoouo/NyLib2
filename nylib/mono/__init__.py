...
class MonoClass(_MonoObj):
    ...
    @functools.cached_property
    def name(self):
        return MonoApi.get_instance().mono_class_get_name(self.ptr)
    ...
    def list_implemented_interfaces(self) -> list:
        api = MonoApi.get_instance()
        it = ctypes.c_size_t(0)
        res = []
...
class MonoImage(_MonoObj):
    ...
    def list_class(self) -> list[MonoClass]:
        api = MonoApi.get_instance()
        res = []
        if api.is_il2cpp:
...
class Mono:
    ...
    def list_domains(self) -> list[MonoDomain]:
        if self.is_il2cpp:
