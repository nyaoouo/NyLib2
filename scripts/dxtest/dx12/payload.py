...
from pyimgui.dx12 import Dx12Inbound
...
def draw(renderer):
    mark("payload_drawn")
    imgui.Begin("pyimgui2 dx12 inbound")
    imgui.Text("DX12 inbound render ok")
    imgui.Text(f"isInLogic={renderer.isInLogic}")
    imgui.End()


renderer = Dx12Inbound(draw)
for attempt in range(50):
    ...
    raise RuntimeError("failed to attach DX12 inbound renderer")
