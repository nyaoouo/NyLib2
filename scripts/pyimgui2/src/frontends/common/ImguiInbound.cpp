...
START_IMGUI_INBOUND_NAMESPACE
{
    ...
    {
        if (g_methodsTable == nullptr || index >= g_methodsTableSize)
...
#define _mSetMethodsTable_(offset, src, size)                                        \
    ...
    void Init(RenderType renderType)
    {
        ...
        case RenderType::D3D12:
        {
            ...
            TestWndClass.hInstance = GetModuleHandle(nullptr);
            TestWndClass.hIcon = nullptr;
            TestWndClass.hCursor = nullptr;
            TestWndClass.hbrBackground = nullptr;
            TestWndClass.lpszMenuName = nullptr;
            TestWndClass.lpszClassName = _T("ImguiInternalTestWndClass");
            TestWndClass.hIconSm = nullptr;
            ...
            HWND TestWnd = ::CreateWindow(
                ...
                WS_OVERLAPPEDWINDOW,
                0, 0, 100, 100,
                nullptr, nullptr, TestWndClass.hInstance, nullptr);
            ...
            {
                HMODULE libD3D9;
                if ((libD3D9 = ::GetModuleHandle(_T("d3d9.dll"))) == nullptr)
                ...
                if ((direct3D9 = ((LPDIRECT3D9(WINAPI *)(UINT))(Direct3DCreate9))(D3D_SDK_VERSION)) == nullptr)
            ...
            case RenderType::D3D10:
            {
                ...
                if ((libD3D10 = ::GetModuleHandle(_T("d3d10.dll"))) == nullptr)
                ...
                if ((mD3D10CreateDeviceAndSwapChain = ::GetProcAddress(libD3D10, "D3D10CreateDeviceAndSwapChain")) == nullptr)
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        if ((hr = ((HRESULT(WINAPI *)(IDXGIAdapter *, D3D10_DRIVER_TYPE, HMODULE, UINT, UINT, DXGI_SWAP_CHAIN_DESC *, IDXGISwapChain **, ID3D10Device **))(mD3D10CreateDeviceAndSwapChain))(
                                 adapter, D3D10_DRIVER_TYPE_HARDWARE, nullptr, 0, D3D10_SDK_VERSION, &swapChainDesc, &swapChain, &device)) != S_OK)
            ...
            case RenderType::D3D11:
            {
                ...
                if ((hr = ((HRESULT(WINAPI *)(_In_opt_ IDXGIAdapter *, D3D_DRIVER_TYPE, HMODULE, UINT, const D3D_FEATURE_LEVEL *, UINT, UINT, const DXGI_SWAP_CHAIN_DESC *, IDXGISwapChain **, ID3D11Device **, D3D_FEATURE_LEVEL *, ID3D11DeviceContext **))(D3D11CreateDeviceAndSwapChain))(
                         nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, featureLevels, 2, D3D11_SDK_VERSION, &swapChainDesc, &swapChain, &device, &featureLevel, &context)) != S_OK)
                        ...
                        do
                        {
                            ...
                            do
                            {
                                ...
                                do
                                {
                                    ...
                                    if ((hr = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator, nullptr, __uuidof(ID3D12GraphicsCommandList), (void **)&commandList)) != S_OK)
    ...
    {
        py::enum_<RenderType>(m, "RenderType", py::module_local())
            .value("None", RenderType::None)
            .value("D3D9", RenderType::D3D9)
            .value("D3D10", RenderType::D3D10)
            .value("D3D11", RenderType::D3D11)
            .value("D3D12", RenderType::D3D12)
            .value("Auto", RenderType::Auto)
            .value("Unknown", RenderType::Unknown);
        m.def("GuessRenderType", &GuessRenderType);
        m.def("Init", &Init, py::arg("renderType"));
        m.def("GetMethod", [](size_t index) { return reinterpret_cast<uintptr_t>(GetMethod(index)); }, py::arg("index"));
        m.def("GetRenderType", &GetRenderType);
    }
